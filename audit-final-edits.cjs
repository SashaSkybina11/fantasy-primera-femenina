...
edit("backend/tests/scoring-prices.test.mjs", (s) =>
  s.replace(
    "{ ...stats, ...create }",
    "{ ...stats, adjustmentPoints: 0, ...create }",
  ),
);
const errors = {
  ...
  "Некорректный диапазон дат тура": [
    "Las fechas de la jornada no son válidas.",
    "Некоректні дати туру.",
  ],
  "Тур или игрок не найден": [
    "No se encontró la jornada o la jugadora.",
    "Тур або гравчиню не знайдено.",
  ],
  "Сначала повторно откройте завершённый тур": [
    "Primero vuelve a abrir la jornada finalizada.",
    "Спочатку повторно відкрийте завершений тур.",
  ],
  ...
  "Тур не завершён": [
    "La jornada todavía no ha finalizado.",
    "Тур ще не завершено.",
  ],
  "Тур или пользователь не найден": [
    "No se encontró la jornada o el usuario.",
    "Тур або користувача не знайдено.",
  ],
  "Нельзя удалить собственный аккаунт администратора": [
    "No puedes eliminar tu propia cuenta de administración.",
    "Не можна видалити власний обліковий запис адміністратора.",
  ],
  "Не удалось создать код приглашения": [
    "No se pudo crear el código de invitación.",
    "Не вдалося створити код запрошення.",
  ],
  ...
  "Некорректный Instagram": [
    "Comprueba el nombre de Instagram.",
    "Перевірте ім’я в Instagram.",
  ],
  "Введите WhatsApp в международном формате": [
    "Introduce WhatsApp en formato internacional.",
    "Введіть WhatsApp у міжнародному форматі.",
  ],
  "Недействительный токен": ["Vuelve a iniciar sesión.", "Увійдіть знову."],
};
edit("frontend/src/services/api.ts", (s) =>
  s.replace(
    ...
    "const apiMessages: Record<string, { es: string; uk: string }> = {\n" +
      Object.entries(errors)
        .map(
          ([k, [es, uk]]) =>
            `  ${JSON.stringify(k)}: ${JSON.stringify({ es, uk })},`,
        )
        .join("\n"),
  ),
);
const keys = {
  ...
  "competition.title": [
    ...
    "Перший дивізіон жіночого футзалу",
  ],
...
edit("frontend/src/contexts/LocaleContext.tsx", (s) =>
  s
    .replace(
      "const spanish = {",
      "const spanish = {\n" +
        Object.entries(keys)
          .map(([k, v]) => `  "${k}": ${JSON.stringify(v[0])},`)
          .join("\n"),
    )
    .replace(
...
for (const p of [
  ...
  edit(`frontend/src/pages/${p}.tsx`, (s) =>
    s
      .replace(
        "Fantasy Primera División<br />Fútbol Sala Femenino",
        '{t("brand.title")}',
      )
      .replace(
      ...
      .replace(
        />Primera División (?:Fútbol Sala Femenino|Femenina)</g,
        '>{t("competition.title")}<',
      )
      .replace(">Fantasy<", '>{t("home.eyebrow")}<')
      .replace(
        'placeholder="you@example.com"',
        'placeholder={t("auth.emailPlaceholder")}',
      ),
  );
edit("frontend/src/layouts/AppShell.tsx", (s) =>
  s
    .replace(/const projectName = \([\s\S]*?\);/, "")
    .replaceAll("{projectName}", '{t("brand.title")}'),
);
edit("frontend/src/contexts/LocaleContext.tsx", (s) =>
  s
    .replace(
