...
test("prelaunch reset, recovery, prices, auth, squad limits, standings and admin leagues", async () => {
  ...
  const { snapshotGameweek, recalculateGameweek } =
    await import("../src/services/gameweeks.js");
  const { getScorers, snapshotThroughGameweek } = await import("../src/services/scorers.js");
  ...
  try {
    ...
    const credentials = {
      email: "old-" + suffix + "@example.invalid",
      password: "Prelaunch-test-123",
    };
      ...
      data: {
        ...
        fantasyTeam: {
          create: {
            name: "Existing FC",
            budget: 1234,
            isInitialSquadComplete: true,
          },
        },
      } as any,
    });
    const oldTeam = await prisma.fantasyTeam.findUniqueOrThrow({
      where: { userId: admin.id },
    });
    const player = await prisma.player.findFirstOrThrow({
      where: { position: "FIELD_PLAYER" },
    });
    const week = await prisma.gameweek.findUniqueOrThrow({
      where: { number: 1 },
    });
    const scoringWeek = await prisma.gameweek.findUniqueOrThrow({ where: { number: snapshotThroughGameweek + 1 } });
    const baselineGoals = (await getScorers(player.clubId)).find(row => row.id === player.id)!.goals;
    await prisma.fantasyTeamPlayer.create({
      data: {
        fantasyTeamId: oldTeam.id,
        playerId: player.id,
        status: "STARTER",
        isCaptain: true,
      },
    });
    await prisma.userGameweekSquad.create({
      data: {
        userId: admin.id,
        gameweekId: week.id,
        fantasyTeamName: "Existing FC",
        players: {
          create: { playerId: player.id, status: "STARTER", isCaptain: true },
        },
      },
    });
    await prisma.userGameweekPoints.create({
      data: {
        userId: admin.id,
        gameweekId: week.id,
        totalPoints: 25,
    ...
    await prisma.userPointAdjustment.create({
      data: {
        userId: admin.id,
        adminId: admin.id,
        gameweekId: week.id,
    ...
    await prisma.gameweekWinner.create({
      data: { userId: admin.id, gameweekId: week.id, rank: 1, points: 25 },
    });
    ...
    await prisma.playerGameweekStats.upsert({
      where: {
        gameweekId_playerId: { gameweekId: scoringWeek.id, playerId: player.id },
      },
      update: { goals: 3, totalPoints: 15 },
      create: {
        gameweekId: scoringWeek.id,
        playerId: player.id,
        goals: 3,
        totalPoints: 15,
      },
    ...
    assert.equal(digest(await snapshot(prisma, gameTables)), digest(before));
    const result = await resetGameUsers(prisma, {
      apply: true,
      backupDirectory: "backups/test",
    });
    ...
    const restore = spawnSync(
      ...
      [
        ...
        "backend/scripts/restore-game-users.ts",
        result.backup!.path,
        "--apply",
      ],
    ...
    for (const row of catalog) {
      const source = seed.find(
        (s) =>
          s.club === row.club.name &&
          s.name === row.name &&
          s.number === row.number,
      )!;
      assert.equal(row.price, source.price);
      assert.ok(Number.isSafeInteger(row.price) && row.price > 0);
    }
    ...
    for (const [position, count] of [
      ...
      for (const p of sorted.filter((p) => p.position === position)) {
        if (chosen.filter((p) => p.position === position).length >= count)
          break;
        if ((clubs.get(p.clubId) ?? 0) >= 2) continue;
        chosen.push(p);
        clubs.set(p.clubId, (clubs.get(p.clubId) ?? 0) + 1);
      }
    ...
    const thirdKeeper = catalog.find(
      (p: any) =>
        p.position === "GOALKEEPER" &&
        !chosen.includes(p) &&
        !chosen.slice(0, 2).some((c) => c.id === p.id),
    );
    await api(
      "/my-team/players",
      fresh.token,
      "POST",
      { playerId: thirdKeeper.id },
      400,
    );
    for (const p of chosen.slice(2))
      await api(
        "/my-team/players",
        fresh.token,
        "POST",
    ...
    assert.equal(full.players.length, 10);
    assert.ok(full.budget >= 0);
    ...
    assert.equal(
      ranking.find((row: any) => row.id === fresh.user.id).totalPoints,
      0,
    );
    const starters = [chosen[0], ...chosen.slice(2, 6)];
    for (const p of starters)
      await api("/my-team/players/" + p.id, fresh.token, "PATCH", {
        status: "STARTER",
      });
    await api("/my-team/captain", fresh.token, "PATCH", {
      playerId: chosen[0].id,
    });
    await api("/my-team/lineup", fresh.token, "PATCH", {
      players: chosen.map((p) => ({
        playerId: p.id,
        status: starters.includes(p) ? "STARTER" : "BENCH",
      })),
    });
    const lineup = await api(
      "/users/" + fresh.user.id + "/lineup",
      login.token,
    );
    assert.equal(lineup.players.length, 5);
    assert.ok(lineup.players.every((p: any) => p.status === "STARTER"));
    assert.deepEqual(Object.keys(lineup.user).sort(), ["id", "name"]);
    assert.ok(
      !/passwordHash|email|token|role.*ADMIN/.test(JSON.stringify(lineup)),
    );
    await prisma.$transaction(async (tx) => {
      await snapshotGameweek(tx, week.id);
      await recalculateGameweek(tx, week.id);
    });
    assert.equal(
      await prisma.userGameweekPoints.count({
        where: { userId: fresh.user.id, gameweekId: week.id },
      }),
      0,
    );
    const clubRoster = await api("/clubs/" + player.clubId + "/players");
    // Rounds 1–3 come from the official snapshot; live corrections start in round 4.
    assert.equal(clubRoster.find((p: any) => p.id === player.id).goals, baselineGoals + 3);
    const payload = {
      started: true,
      result: "WIN",
      goals: 2,
    ...
    await api(
      "/admin/gameweeks/" + scoringWeek.id + "/players/" + player.id + "/stats",
      login.token,
      "PUT",
      payload,
    );
    assert.equal(
      (await api("/clubs/" + player.clubId + "/players")).find(
    ...
    assert.ok(
      (await api("/admin/friend-leagues", login.token)).some(
        (l: any) => l.id === league.id,
      ),
    );
    const stateBeforeDelete = digest(await snapshot(prisma, gameTables));
    await api(
      "/admin/friend-leagues/" + league.id,
      login.token,
      "DELETE",
      undefined,
      204,
    ...
    assert.equal(
      await prisma.privateLeagueMember.count({
    ...
    writeFileSync(
      "artifacts/prelaunch/integration.json",
      JSON.stringify(
        {
          passed: true,
          playersChecked: catalog.length,
          reset: result,
          scenarios: [
            ...
            "11th player rejected",
            "10-player ranking",
            "no retroactive scoring",
            "private-safe starters",
            "goals correction",
            "403 admin guard",
            "league cascade isolation",
          ],
        },
        null,
        2,
