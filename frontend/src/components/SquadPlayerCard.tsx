...
export function SquadPlayerCard({ entry, readOnly = false, onMove, onCaptain, onRemove }: { entry: SquadEntry; readOnly?: boolean; onMove?: () => void; onCaptain?: () => void; onRemove?: () => void }) {
  ...
  return (
    <article className="squad-player-card">
      ...
      {!readOnly && <div className="squad-actions">
        ...
        <button className="text-button text-button--danger" onClick={onRemove}>{t("squad.remove")}</button>
      </div>}
    </article>
  );
}
