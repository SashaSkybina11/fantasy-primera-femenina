  ...
  return (
    <>
      {navigation.map((item) => (
        ...
        >
          <i>
            <FootballIcon name={item.icon} />
          </i>
          <span>{mobile ? (item.mobile ?? item.label) : item.label}</span>
        </NavLink>
      ))}
    </>
  );
...
export function AppShell() {
  ...
  const requestSignOut = (event: React.MouseEvent<HTMLButtonElement>) => {
    logoutTrigger.current = event.currentTarget;
    setMenuOpen(false);
  ...
  useEffect(() => {
    ...
    const closeOnEscape = (event: KeyboardEvent) => {
      if (event.key === "Escape") setLogoutDialogOpen(false);
    };
        ...
        >
          ...
          >
            ...
            <div className="logout-modal__actions">
              <button className="logout-modal__confirm" onClick={signOut}>
                {t("logout.confirm")}
              </button>
              <button
