...
export function AdminMatchesPage() {
  ...
  return <div className="page page--narrow"><header className="page-heading"><p className="eyebrow">{t("nav.admin")}</p><h1>{t("matches.admin")}</h1><p>{t("matches.editorHelp")}</p><Link to="/admin/player-points">{t("matches.legacy")} →</Link><Link to={`/results?week=${week}`}>{t("matches.title")} →</Link></header>
    ...
    <form className="admin-card match-create" onSubmit={e => { e.preventDefault(); create.mutate(); }}><h2>{t("matches.create")}</h2><div className="match-create-fields">{(["home", "away"] as const).map(side => <label key={side}>{t(`matches.${side}`)}<select required value={side === "home" ? homeId : awayId} onChange={e => side === "home" ? setHome(e.target.value) : setAway(e.target.value)}><option value="">—</option>{clubs.data?.filter(c => !used.has(c.id) && c.id !== (side === "home" ? awayId : homeId)).map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label>)}<label>{t("matches.kickoff")}<input type="datetime-local" value={kickoff} onChange={e => setKickoff(e.target.value)} /></label></div><button className="button" disabled={!week || !homeId || !awayId || create.isPending || matches.isPending || weeks.data?.find(w => w.id === week)?.status === "COMPLETED"}>{t("matches.create")}</button></form>
...
function MatchEditor({ data }: { data: MatchEditorData }) {
  ...
  const numberField = (key: "homeScore" | "awayScore" | "homeOwnGoals" | "awayOwnGoals", value: string) => { setDirty(true); setProtocol(old => ({ ...old, [key]: value === "" && key.endsWith("Score") ? null : Number(value) })); };
