...
export function AdminPlayerPricesPage() {
  ...
  const busy = calculate.isPending || apply.isPending;
  const components = [
  ...
  return <div className="page admin-prices">
    ...
    <section className="admin-toolbar">
      ...
      <button className="button" disabled={!gameweekId || busy} onClick={() => { setPreview(null); calculate.mutate(gameweekId); }}>{t("prices.calculate")}</button>
    </section>
    {(players.isError || weeks.isError) && <p role="alert">{t("error.generic")}</p>}
