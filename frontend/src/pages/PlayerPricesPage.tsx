...
export function PlayerPricesPage() {
  ...
  return <div className="page">
    ...
    <div className="catalog-filters"><input value={search} onChange={event => setSearch(event.target.value)} placeholder={t("adminStats.searchPlaceholder")} aria-label={t("adminStats.searchPlaceholder")} />
    <select value={clubId} onChange={event => setClubId(event.target.value)} aria-label={t("player.club")}><option value="">{t("player.allClubs")}</option>{clubs.map(club => <option key={club.id} value={club.id}>{club.name}</option>)}</select></div>
    {prices.isPending && <p><Loader label={t("loading.players")} /></p>}
    {prices.isError && <p role="alert">{t("error.generic")}</p>}
    <div className="price-preview-grid">{prices.data?.filter(player => (!clubId || player.club?.id === clubId) && `${player.name} ${player.club?.name}`.toLowerCase().includes(search.toLowerCase())).map(player => <article className="admin-card player-price-card" key={player.id}>
