...
export function MatchHeading({ match }: { match: MatchRecord }) {
  ...
  return <div className="match-score"><div><ClubLogo club={home.club} /><strong>{home.club.name}</strong></div><b>{match.published ? `${match.published.homeScore} : ${match.published.awayScore}` : "— : —"}</b><div><ClubLogo club={away.club} /><strong>{away.club.name}</strong></div></div>;
}
export function ResultsPage() {
  ...
  return <div className="page page--narrow"><header className="page-heading"><p className="eyebrow">{t("competition.title")}</p><h1>{t("matches.title")}</h1><p>{t("matches.intro")}</p>{user?.role === "ADMIN" && <Link to={`/admin/matches?week=${week}`}>{t("matches.admin")} →</Link>}</header>
    ...
    <div className="match-list">{matches.data?.map(match => <Link className="match-card" to={`/results/${match.id}`} key={match.id}><small>{match.kickoffAt ? new Date(match.kickoffAt).toLocaleString(locale, { dateStyle: "medium", timeStyle: "short" }) : t("matches.unscheduled")}</small><MatchHeading match={match} /><span className="match-card-footer">{match.published ? t("matches.published") : t("matches.pending")}<span>{t("matches.report")} →</span></span></Link>)}</div>
  </div>;
}
