...
const results = [];
const failures = [];
for (const locale of ["uk"]) for (const theme of ["light", "dark"]) {
  ...
  for (const width of [320, 1280]) {
    ...
    for (const path of ["/admin/player-points", "/admin/player-prices", "/purchase-players", "/friend-leagues"]) {
      ...
      if (path === "/admin/player-points") await page.locator(".stats-player").first().scrollIntoViewIfNeeded();
      if (path === "/admin/player-prices") await page.locator(".price-table").scrollIntoViewIfNeeded();
      if (path === "/purchase-players") await page.locator(".popular-player").scrollIntoViewIfNeeded();
      const overflow = await page.evaluate(() => document.documentElement.scrollWidth > innerWidth + 1);
      if (overflow) failures.push(`Horizontal overflow ${locale}/${theme}/${width}${path}`);
      if ([320, 768, 1280].includes(width) && locale === "uk") await page.screenshot({ path: fileURLToPath(new URL(`detail-${theme}-${width}-${path.replaceAll("/", "_")}.png`, out)), fullPage: false });
      if (path === "/rules") {
...
await browser.close();
await writeFile(new URL("visual-review-results.json", out), JSON.stringify({ results, failures }, null, 2));
